import random, math, weather, player, team, discord, asyncio, json, time
...
class Game(object):
    ...
    def at_bat(self):
        ...
        self.weather.modify_atbat_roll(update, roll, defender)
    ...
    def steals_check(self, attempts):
        ...
        for baserunner, start_base in attempts:
            ...
            if run_roll < 1:
                ...
                self.outs += 1
                results_text.append(random.choice(self.voice["steal_caught"]))
            else:
                self.bases[start_base+1] = baserunner
                results_text.append(random.choice(self.voice["steal_success"]))
    ...
    def batterup(self):
        ...
        self.weather.activate(self, update)
        ...
        self.weather.post_activate(self, update)

        if self.outs >= 3:
            self.flip_inning()

        return update #returns ab information and scores
...
async def startgame(inter: discord.Interaction, home: str, away: str, weather_name: str = "None", voice: str = None, innings: int = 9):
    ...
    if voice is not None and voice not in [v["name"] for v in all_voices]:
...
async def prepare_game(newgame):
    ...
    if newgame.voice is None:
        newgame.voice = random.choice(all_voices)
